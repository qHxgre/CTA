...
Logs
1. 修复：修复重复发单的bug
2. 增加一个发平仓单的功能：即时发平仓单
    a. 设置一个close_grid，该值是根据在 open_orders 中找未完全平仓的开仓单网格的最小值（short）/最大值（long）设置的
    b. 触发 close_grid 时（偏移1个点），发仓单，注意：发单时要检查是否重复发单
3. 修复开仓撤单导致网格间距过大的bug
4. 优化网格间距和手数的变化
"""

class future_grid_inf_v104(CtaTemplate):
    """期货网格策略"""
    vtSymbol = ''
    exchange = ''
    className = 'future_grid_inf_v104'
    author = 'hxgre'
    ...
    def read_overnight_position(self,):
        ...
        if (overnight is None) or (overnight.shape[0] == 0):
            ...
            self.write_log(
                "\n There are overnight positions. "
                + f"Total volume: {total_volume}, grids: {grids}."
            )
    ...
    def read_base_position(self):
        ...
        if (base is None) or (base.shape[0] == 0):
            ...
            self.write_log(f"\n TEST: {self.variables.base_orders}.")
            total_volume = 0
            for _, v in self.variables.base_orders.items():
                total_volume += v.traded_volume
            self.write_log(f"\n There are base positions. Total volume: {total_volume}.")
    ...
    def process_close_overnight(self, curtime: str):
        ...
        if cond_1 | cond_2:
            return
        self.write_log('\n Enter the process of closing the overnight position. ###############')
        self.close_grid(method='overnight')
        self.save_position("grid")
        self.variables.process_close_overnight_executed = True
        self.write_log('\n Finish the process of closing overnight position. ###############')
    ...
    def calc_open_grids(self, curr_ask: float, curr_bid: float):
        ...
        if ((self.variables.gridlines[DIRECTION_SHORT]['next_grid'] !=0 ) &
            ...
            if self.check_send_open(DIRECTION_SHORT, curr_ask):
                order_info = self.send_order({
                    'direction': DIRECTION_SHORT,
                    'offset': OFFSET_OPEN,
        ...
        if ((self.variables.gridlines[DIRECTION_LONG]['next_grid'] !=0 ) &
            ...
            self.update_qty(self.variables.gridlines[DIRECTION_LONG]['next_grid'])
            if self.check_send_open(DIRECTION_LONG, curr_bid):
                order_info = self.send_order({
                    'direction': DIRECTION_LONG,
                    'offset': OFFSET_OPEN,
    ...
    def update_qty(self, curr_grid: float):
        ...
        self.write_log(f"\n curr price: {curr_grid}, vol_sum: {vol_sum}, remaining qty: {remaining_qty}, curr interval: {curr_interval}")
    ...
    def check_send_open(self, direction: int, price: float) -> bool:
        """发单时的检查逻辑
        1. 重复发单：check_double_send
        2. 资金不足：check_account_balance
        """
        cond_1 = self._check_double_send(price)
        cond_2 = self._check_account_balance()
        cond_3 = self._check_grid_edge(direction, price)
        return cond_1 & cond_2 & cond_3

    def _check_double_send(self, price: float) -> bool:
        """检查是否重复发单"""
        for _, v in self.variables.open_orders.items():
            if round(float(v.order_price), 2) == round(float(price), 2):
                if v.status != STATUS_ALLTRADED:
                    self.write_log(f'Prohibit send! Reason: grid {v.order_price} is already exist!')
                    return False        # 该网格有挂单未成交
        return True
    ...
    def _check_account_balance(self) -> bool:
        ...
        if account_balance < 10000:
            self.write_log(f'Prohibit send! Reason: balance to send open orders! {account_balance}')
            return False
        return True

    def _check_grid_edge(self, direction: int, price: float) -> bool:
        """检查网格边际"""
        if (direction == DIRECTION_LONG) & (price < self.strategy_parameters.long_min_price):
            # self.write_log(f'Prohibit send! Reason: beyong edge of long, {price} < {self.strategy_parameters.long_min_price}.')
            return False
        if (direction == DIRECTION_SHORT) & (price > self.strategy_parameters.short_max_price):
            # self.write_log(f'Prohibit send! Reason: beyong edge of short, {price} > {self.strategy_parameters.short_max_price}.')
            return False
        return True
    ...
    def send_order(self, place_order: dict):
        ...
        if order_info.offset == OFFSET_OPEN:
            ...
            self.write_log(f'\n Update grid lines after sending an open order: {self.variables.gridlines}')
        return order_info
    ...
    def close_grid(self, trade=None, method: str='intraday'):
        ...
        if method == 'intraday':
            if self.strategy_parameters.close_position_method == CLOSE_ADVANCED:
                self.write_log('\n Close grid in advance.')
                self._close_grid_in_advance(trade)
            elif self.strategy_parameters.close_position_method == CLOSE_PROMPTLY:
                self.write_log('\n Close grid promptly.')
                self._close_grid_promptly()
            else:
                self.write_log(
                    f'Wrong method of closing position: {self.strategy_parameters.close_position_method}'
                )
            ...
            for k, v in self.variables.open_orders.items():
                if int(v.order_id) >= 0:     # 隔夜持仓的 order_id 统一为负数
                    continue
                if self.strategy_parameters.close_position_method == CLOSE_ADVANCED:
                    self.write_log("Close overnight positions in advance. ")
                    self._close_overnight_in_advance(k, v)
                elif self.strategy_parameters.close_position_method == CLOSE_PROMPTLY:
                    self.write_log("Close overnight positions promptly. ")
    ...
    def _close_grid_in_advance(self, trade):
        """盘中交易-提前平仓"""
