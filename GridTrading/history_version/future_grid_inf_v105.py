...
class future_grid_inf_v105(CtaTemplate):
    """期货网格策略"""
    vtSymbol = ''
    exchange = ''
    className = 'future_grid_inf_v105'
    author = 'hxgre'
    ...
    def onTimer(self, tid: int) -> None:
        ...
        if tid == 1:
            ...
            self.strategy_parameters.update_grid_interval(10)
            # self.strategy_parameters.update_close_short(indicate.params['interval'])
            self.strategy_parameters.update_close_short(10)
            # self.strategy_parameters.update_close_long(indicate.params['interval'])
            self.strategy_parameters.update_close_long(10)
            self.write_log(
